...
async def test_create_task(
    ...
    if client.authorized and client.tes_resp_code != 500:
        mock_arborist_request.assert_any_call(
            method="POST",
            path=f"/auth/request",
            body=f'{{"requests":[{{"resource":"/services/workflow/gen3-workflow/tasks/{TEST_USER_ID}","action":{{"service":"gen3-workflow","method":"read"}}}}],"user":{{"token":"{TEST_USER_TOKEN}"}}}}',
            authorized=client.authorized,
        )
...
async def test_create_task_new_user(client, access_token_patcher, mock_aws_services):
    ...
    test_task_body = {
        "name": "test-task",
        "tags": {
            "_AUTHZ": f"/services/workflow/gen3-workflow/tasks/{NEW_TEST_USER_ID}/TASK_ID_PLACEHOLDER",
...
async def test_create_task_with_reserved_tags(client, access_token_patcher):
    ...
    res = await client.post(
        "/ga4gh/tes/v1/tasks",
        json={"name": "test-task", "tags": {"_authz": "custom-authz-value"}},
        headers={"Authorization": f"bearer {TEST_USER_TOKEN}"},
    )
...
async def test_delete_task(client, access_token_patcher, trailing_slash):
    ...
    # there is always a 1st call with view=FULL to get the _AUTHZ tag
