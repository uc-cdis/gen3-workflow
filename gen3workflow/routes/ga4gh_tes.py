...
async def create_task(request: Request, auth=Depends(Auth)) -> dict:
    ...
    # TODO: Return a 400 error instead of 500 when there is a decode exception caused by a bad
    # command value while getting request body
...
async def cancel_task(request: Request, task_id: str, auth=Depends(Auth)) -> dict:
    ...
    res = await make_tes_server_request(request.app.async_client, "post", url)
